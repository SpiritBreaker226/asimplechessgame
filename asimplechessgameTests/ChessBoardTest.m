...
- (void)testSetCellState_withValidCoords_cellStateShouldChange {
	...
	[_testChessBoard setCellState:cellChessPieceCannotMoveHere OnRow:3 andColumn:6];
	
	// It should now be find the current state for that location
	XCTAssertEqual(cellChessPieceCannotMoveHere, [_testChessBoard getCurrentStateAtRow:3 andColumn:6], @"This cell Chess Piece cannot be moved here");
}// end of testSetCellState_withValidCoords_cellStateShouldChange()

- (void)testSetCellState_withInvalidCoords_cellShouldErrorOut {
	// it should error out
	XCTAssertThrows([_testChessBoard setCellState:cellHasChessPiece OnRow:20 andColumn:8], @"An excetion should have been rised for this cell");
